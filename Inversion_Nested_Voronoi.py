...
from Imshow_Data import Imshow_Data
from scipy.signal import lfilter
from datetime import datetime
from Imshow_AllChain import Imshow_AllChain
...
KminAR = 0
KmaxAR = 3
Chain = np.zeros(10+2*KmaxAR+Kmax*3).astype('float32')
...
if rank == 0:
    ...
    Gravity_Data = np.loadtxt('GRV_Profile.txt').astype('float32')
    Magnetic_Data = np.loadtxt('RTP_Profile.txt').astype('float32')
    ...
    XnZn = np.column_stack((Xn,Zn)).astype('float32')
   ...
   # Adding noise
    ...
    AR_parameters_original_g = np.array([0.6,-0.5]).astype('float32')
    #AR_parameters_original_g = np.array([0.0]).astype('float32')
    noise_g_level = 0.08
    sigma_g_original=noise_g_level*max(abs(dg_true))
    uncorr_noise_g_original = sigma_g_original*np.random.randn(Ndatapoints).astype('float32')
...
#         dg_obs = np.load(fpath_loaddesk+'//'+'dg_obs_08.npy') # use this, if the latest result exists 
    ...
    AR_parameters_original_T = np.array([0.]).astype('float32')
    noise_T_level = 0.04
    sigma_T_original=noise_T_level*max(abs(dT_true))
    uncorr_noise_T_original = sigma_T_original*np.random.randn(Ndatapoints).astype('float32')
...
#         dT_obs = np.load(fpath_loaddesk+'//'+'dT_obs.npy') # use this, if the latest result exists 
    ...
    globals_par = np.matrix([[Kmin, Kmax], [rho_salt_min, rho_salt_max], [rho_base_min, rho_base_max], [KminAR, KmaxAR],[zn_min,sus_base_max]])
...
if rank > 0:
    ...
    for imcmc in np.arange(1,NMCMC+1):  # 1 to NMCMC
        LogLc = Chain[0].copy()
        [xmc, zmc, xc, zc, rhoc, ARgc, ARTc]= Chain2xz(Chain)

        
        if imcmc % 4 != 0:
        
            for istep in np.arange(1,np.random.randint(1,4)):
          
                step = select_step(globals_par[0,0],globals_par[0,1],np.size(xc),bk_Nodes)
        
                if step==91:
                    [LogLc,xc,zc,rhoc] = birth(XnZn,globals_par,LogLc,xmc,zmc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs)
                elif step==92:
                    [LogLc,xc,zc,rhoc] = death(XnZn,LogLc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs)
                else:
                    [LogLc,xmc,zmc,xc,zc,rhoc] = move(XnZn,globals_par,LogLc,xmc, zmc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs)
        
        elif imcmc>50000 and imcmc % 4 == 0:
            [LogL, model_vec_g, model_vec_T, rg, rT, sigma_g, sigma_T, uncor_g, uncor_T] = Log_Likelihood(Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,xc,zc,rhoc,ARgc,ARTc,XnZn)
            
            if sigma_g<= 1.0 and sigma_T<=4.0:
                ...
                if step==91:
                    [LogLc,ARgc] = birth_ARg(XnZn,AR_bounds,LogLc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,bk_AR)
                elif step==92:
                    [LogLc,ARgc] = death_ARg(XnZn,LogLc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,bk_AR)
                else:
                    [LogLc,ARgc] = move_ARg(XnZn,AR_bounds,LogLc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs)
                ...
                if step==91:
                    [LogLc,ARTc] = birth_ART(XnZn,AR_bounds,LogLc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,bk_AR)
                elif step==92:
                    [LogLc,ARTc] = death_ART(XnZn,LogLc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,bk_AR)
                else:
                    [LogLc,ARTc] = move_ART(XnZn,AR_bounds,LogLc,xc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs)
        ...
        Chain[4+np.size(ARgc)+np.size(ARTc):10+np.size(ARgc)+np.size(ARTc)+np.size(xc)*3] = np.concatenate((xmc,zmc,xc,zc,rhoc)).copy()
...
## MASTER rank == 0
    ...
    with open(ChainHistory_str,"wb") as f:
        ...
        for imcmc in np.arange(1,NMCMC+1):  # 1 to NMCMC
            ...
            if np.isin(ChainAll[:,2:4], 1).all() and np.isin(ChainAll[:,4:6], 0.).all():
            ...
            if ikeep == NKEEP:
                ...
                #if raw == 1:np.save(Chain_raw_str, Chain_raw)
                
                c += 1
                if c % 100 == 0:
                    #Imshow_BestChain(dis_min/1000,dis_max/1000,z_min/1000,z_max/1000,XnZn,CX,CZ,globals_par,ChainAll,fpath_PDF,'MaxLG_',c)
                    Imshow_AllChain(dis_min/1000,dis_max/1000,z_min/1000,z_max/1000,XnZn,CX,CZ,globals_par,ChainAll,fpath_PDF,'AllChain',c)
                    
                    #Imshow_Data(dis_s,dg_obs,dT_obs,XnZn,Kernel_Grv,Kernel_Mag,ChainAll[-5:-1,:],fpath_PDF,'Data_',c)
