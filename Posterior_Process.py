...
Ncol = 10+2*KmaxAR+Kmax*3
ndelete = 2000000 # must be dividable by Nworkers (e.x. 10, 100, ...)
Nchain = 50000   # numder of chains(rows) per each worker
...
for ichain in np.arange(Nchain):
        ...
        [xm, zm, x, z, rho, ARg, ART]= Chain2xz(Chain).copy()
        
        [LogL, model_vec_g, model_vec_T, rg, rT, sigma_g, sigma_T, uncor_g, uncor_T] = Log_Likelihood(Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,x,z,rho,ARg,ART,XnZn)
        ...
        sigmakeep_g[ichain] = sigma_g.copy()
        sigmakeep_T[ichain] = sigma_T.copy()
